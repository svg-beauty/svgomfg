...
addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        cacheNames
          .filter((cacheName) => cacheName.startsWith(cachePrefix) && !expectedCaches.has(cacheName))
...
async function handleFontRequest(request) {
  ...
  const [response, fontCache] = await Promise.all([fetch(request), caches.open(fontCacheName)])
